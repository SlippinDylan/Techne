...
Основные рабочие процессы реализованы. Для каждого push и pull request запускаются тесты автоматизации, `TechneTests` и неподписанная сборка Release. Публикация в текущем манифесте выпуска отключена.

## Платформа

| Параметр | Значение |
|---|---|
| Минимальная версия macOS | macOS 15.0 Sequoia |
| Сборка из исходников | macOS 15.0+ и Xcode 26+ |
| Архитектуры текущей сборки | `arm64` и `x86_64` (Apple Silicon и Intel) |
