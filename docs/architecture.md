...
Techne 对自己启动的任务保留受管执行记录。停止时先获取进程快照，再取消当前受管任务，并且只清理同时匹配项目目录与当前启动命令特征的开发服务进程组。项目目录只限定搜索范围，不能单独证明进程归 Techne 管理。应用启动和全局刷新只获取一次系统进程快照，并通过相同规则恢复 Shell 项目的运行状态；不得为每个项目重复执行全系统扫描。

项目的启动与重新启动共享 replace-and-start 语义：先确认该项目受管的旧开发服务进程组全部停止，停止失败时禁止新启动。原生微信小程序不参与系统进程推断；用户明确停止或重新启动项目时通过微信开发者工具 CLI 执行 close 或 close/open，退出 Techne 不关闭微信开发者工具或其中已打开的项目。

缓存 PID 和项目工作目录都只是提示，不能单独作为终止进程的充分依据。停止范围必须限定在目标项目及其当前启动命令，无法建立归属时不发送信号。
