...
メインウインドウを閉じても、プロジェクトはメニューバーで実行を継続します。ステータスメニューから「Techne を終了」を選ぶと、管理対象の Shell 開発サービスだけを安全に停止し、WeChat Developer Tools などの外部アプリケーションは開いたままにします。Techne の起動時にはプロジェクトパスと起動コマンドに基づいて存続している Shell プロジェクトの状態を復元し、開始操作では一致する管理対象サービスのプロセスグループだけを置き換えます。
