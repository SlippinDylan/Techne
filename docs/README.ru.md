...
Основные рабочие процессы реализованы. Для push в main и pull request запускаются лёгкие проверки автоматизации. Если изменены только `README.md`, `docs/`, `LICENSE` или `AGENTS.md`, сборка macOS пропускается, кроме случаев, когда публикация включена; для остальных изменений запускаются `TechneTests` и неподписанная сборка Release. Публикация в текущем манифесте отключена.
